...
class ManualDups
  class Index
    ...
    def item_params(item)
      ...
      original_img_height ||= DEFAULT_THUMB_WIDTH / 2

      dup_img_height = DEFAULT_THUMB_WIDTH * d['height'] / d['width']
      dup_img_height = original_img_height unless dup_img_height.positive?
    ...
    def get_default_action(o, d)
      return 'orig-nofoto-dup-remove' if o['full_path'].include?('nofoto') || d['full_path'].include?('nofoto')

      return 'orig-alien-dup-remove' if o['full_path'].include?('alien') || d['full_path'].include?('alien')
