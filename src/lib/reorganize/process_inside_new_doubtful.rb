...
require './lib/media'
require './lib/reorganize/process_inside_new_doubtful/base'
require './lib/reorganize/process_inside_new_doubtful/image'
require './lib/reorganize/process_inside_new_doubtful/video'
...
class ProcessInsideNewDoubtful
  ...
  def call(data)
    ...
    # dup: /app/video_new/takeout-katia-202306-video/Photos from 2018/VID-20181231-WA0015.mp4
    # len: 57.91, ratio: 1.0
    # dup: /app/video_new/takeout-katia-202306-video/Photos from 2018/VID-20190128-WA0017.mp4
  ...
  def generate_actions(files, data, already_processed_files)
    ...
    files.each do |key, dups|
      ...
      cluster.each do |file_name|
        ...
        root_dups_dir = File.join(@dups_dir, 'new_inside_doubtful')
        new_file_name = file_name.to_s.gsub(@new_dir, root_dups_dir)
        ...
        actions << {
          type: 'move',
          from: data[file_name].merge(full_path: file_name.to_s),
          to: new_file_name,
          original: data[best].merge(full_path: best.to_s),
        }
