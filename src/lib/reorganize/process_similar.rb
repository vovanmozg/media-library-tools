class ProcessSimilar
  ...
  def generate_actions(similar, existing_by_phash)
    ...
    similar.each do |file_name, file_info|
      original_file = existing_by_phash[file_info[:phash]].keys.first
      root_dups_dir = File.join(@dups_dir, 'new_similar')
      ...
      actions << {
        ...
        to: new_file_name,
        original: existing_by_phash[file_info[:phash]][original_file].merge(full_path: original_file.to_s)
      }
