class ProcessFullDups
  ...
  def generate_actions(full_dups, existing_by_md5)
    ...
    full_dups.each do |file_name, file_info|
      file_name = file_name.to_s
      original_file = existing_by_md5[file_info[:partial_md5]].keys.first
      root_dups_dir = File.join(@dups_dir, 'new_full_dups')

      new_file_name = file_name.to_s.gsub(@new_dir, root_dups_dir)

      actions << {
        type: 'move',
        from: file_info.merge(full_path: file_name.to_s),
        to: new_file_name,
        original: existing_by_md5[file_info[:partial_md5]][original_file].merge(full_path: original_file.to_s)
      }
