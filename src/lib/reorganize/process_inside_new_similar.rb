class ProcessInsideNewSimilar
  ...
  def generate_actions(new_inside_full_dups, data)
    ...
    new_inside_full_dups.each do |file_name, groups|
      root_dups_dir = File.join(@dups_dir, 'new_inside_similar')
      ...
      actions << {
        ...
        to: new_file_name,
        original: data[groups[:original]].merge(full_path: groups[:original].to_s),
      }
    end
