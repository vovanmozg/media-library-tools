...
describe ProcessInsideNewDoubtful do
  ...
  it 'returns empty list if phash differs a lot' do
    files_to_processing = {
      '/new/x/1.jpg' => image,
      '/new/x/2.jpg' => image(name: '2.jpg', phash: 787_907_979_500_066_548)
    }
  ...
  it 'returns empty list if ratio differs a lot' do
    files_to_processing = {
      '/new/x/1.jpg' => image,
      '/new/x/2.jpg' => image(name: '2.jpg', width: 300)
    }
  ...
  it 'does not find dups in groups' do
    ...
    expected = {
      inside_new_doubtful: [
        {
          type: 'move',
          original: image1.merge(full_path: '/new/x/1.jpg'),
          from: image2.merge(full_path: '/new/x/2.jpg'),
          to: '/dups/new_inside_doubtful/x/2.jpg',
        },
        {
          type: 'move',
          original: image1.merge(full_path: '/new/x/1.jpg'),
          from: image3.merge(full_path: '/new/x/3.jpg'),
          to: '/dups/new_inside_doubtful/x/3.jpg',
        }
      ]
    }
    ...
    actions, processed_new_files, error_files = subject.call(files_to_processing)
    expect(actions).to eq(expected)
    expect(processed_new_files).to eq(%w[/new/x/2.jpg /new/x/3.jpg])
    expect(error_files).to eq([])
  end
  ...
  it 'makes file with largest dimensions as original' do
    ...
    expected = {
      inside_new_doubtful: [
        type: 'move',
        original: image2.merge(full_path: '/new/x/2.jpg'),
        from: image1.merge(full_path: '/new/x/1.jpg'),
        to: '/dups/new_inside_doubtful/x/1.jpg'
      ]
    }
  ...
  it 'returns longest video if duplicates with slightly different length' do
    ...
    expected = {
      inside_new_doubtful: [
        type: 'move',
        original: video1.merge(full_path: '/new/x/1.mp4'),
        from: video2.merge(full_path: '/new/x/2.mp4'),
        to: '/dups/new_inside_doubtful/x/2.mp4',
      ]
    }

    actions, processed_new_files, error_files = subject.call(files_to_processing)
    expect(actions).to eq(expected)
    expect(processed_new_files).to eq(['/new/x/2.mp4'])
    expect(error_files).to eq([])
  end
  ...
  it 'returns action if duplicates with phash = 3' do
    ...
    expected = {
      inside_new_doubtful: [
        type: 'move',
        from: video1.merge(full_path: '/new/x/1.mp4'),
        to: '/dups/new_inside_doubtful/x/1.mp4',
        original: video2.merge(full_path: '/new/x/2.mp4'),
      ]
    }

    actions, processed_new_files, error_files = subject.call(files_to_processing)
    expect(actions).to eq(expected)
    expect(processed_new_files).to eq(['/new/x/1.mp4'])
    expect(error_files).to eq([])
  end
end
